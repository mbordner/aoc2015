...
class Player
  ...
  def effect_remaining_time(spell)
    @effects.each do |e|
...
class GameState
  ...
  def effect_remaining_time(name)
    return [@player.effect_remaining_time(name), @boss.effect_remaining_time(name)].max
  end
...
def get_cheapest_spell_options
  ...
  get_cheapest_spell_options_rec = lambda { |spell, state|
    ...
    next_spell_options.each do |ns|
      if state.effect_remaining_time(ns) <= 1
...
=begin
g = GameState.new(true)
g.turn_with_player_spell($spells[4])
g.turn_with_player_spell($spells[2])
g.turn_with_player_spell($spells[1])
g.turn_with_player_spell($spells[3])
g.turn_with_player_spell($spells[0])
=end
